...
const AddClientsEvent = (props: any) => {
  ...
  const userItems = props.addedUsers.map((u: any) => (
    <UserItem
      id={u.id}
      addedUsers={props.addedUsers}
      setAddedUsers={props.setAddedUsers}
      name={u.name}
    />
  ));
  const token = useTypedSelector(state => state.user.token);
  const setName = (e: any) => setGroupName(e.target.value);
  const createGroupUsers = () => {
    const users: any[] = props.addedUsers;
    const arr: any[] = [];
  ...
  if (props.change) {
    return (
      <div className={s.cont}>
        ...
        <div onClick={createGroupUsers} className={s.btn}>
          Создать группу
        </div>
      </div>
    );
  ...
  return (
    <div className={s.cont}>
      ...
      <div onClick={changeGroupUsers} className={s.btn}>
        Сохранить изменения
      </div>
    </div>
  );
