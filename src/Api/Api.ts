...
const Api = {
    ...
    getUsers(token: string, page: number, perPage: number) {
        return instance.get(`users?token=${token}&page=${page}&per-page=${perPage}`)
    },
    editUser(token: string, argName: string, userId: number, value: string) {
        return instance.post(`edit-user?token=${token}`, {'arg_name': argName, 'user_id': userId, 'new_value': value})
    },
    addUser(token: string, name: string, email: string, phone: number, tagId: string, network: string, role: string) {
